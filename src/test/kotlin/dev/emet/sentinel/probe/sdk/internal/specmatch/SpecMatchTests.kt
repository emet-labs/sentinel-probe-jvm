...
class SpecMatchTests {
    ...
    fun `table of selection cases`() {
        data class Case(
            val name: String,
            val spec: SpecificationFilter?,
            val event: ProducerEvent?,
            val want: Boolean,
        )
        ...
        val cases =
            listOf(
                ...
                Case(
                    "kinds are exact, not prefixes",
                    withKinds("order"),
                    ProducerEvent.newBuilder().setKind("order.charged").build(),
                    false,
                ),
                Case(
                    "no EventMatch selects defensively",
                    SpecificationFilter.newBuilder().build(),
                    ProducerEvent.newBuilder().setKind("x").build(),
                    true,
                ),
